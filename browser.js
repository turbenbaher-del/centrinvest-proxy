...
// ─── Подпись документа ────────────────────────────────────────────────────────
// Живёт в отдельном модуле `sign.js`: подпись идёт ЧЕРЕЗ ФОРМЫ банка, тем же
// путём, что и веб-версия (client/cryptoProfileSelect → client/eTokenPassSign →
// client/smsSign → client/signResult). Прежняя реализация звала REST
// prepareSign/continueSign — банк отвечал на верный код «Введен неверный
// пароль», потому что код уходил в чужой конвейер.
...
// Модуль рублёвых платёжных поручений в REST банка. Нужен для чтения списка,
// печатной формы и удаления документов; сама подпись идёт через формы (sign.js).
