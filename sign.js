...
/**
 * Последняя подходящая ФОРМА в ответе банка.
 *
 * Именно formInit, а не «всё, кроме formClose». Банк присылает вокруг форм
 * служебные команды с теми же именами: `touchFields`, `stateUpdate`,
 * `formClose`. Поймано дважды на живых прогонах 13.08.2026: на выделение
 * строки прилетал `stateUpdate` без кнопок и сервис терял действие подписи,
 * а после выбора средства — `touchFields` по закрытому окну выбора, и сервис
 * решил, что банк не предложил ни одного средства, хотя в том же ответе уже
 * лежала форма ввода ключа с токена.
 *
 * Форму берём последнюю: на каждое переключение банк закрывает старую и
 * открывает новую с тем же именем и НОВЫМ токеном.
 */
